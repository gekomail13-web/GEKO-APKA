...
def load_clients():
    try:
        # Czytamy plik CSV. Separator to średnik (standard w polskim Excelu)
...
def wyslij_maila(klient_info, brakuje, rekomendacja, email_nadawcy, haslo_nadawcy, email_odbiorcy):
    ...
    msg['Subject'] = f"🔔 OKAZJA: {klient_info['Nazwa']} - Brakuje {brakuje:.2f} zł!"
    
    tel_link = f"tel:{klient_info['Telefon']}" if klient_info['Telefon'] else "#"
    
    body = f"""
    Cześć! Analiza zamówienia.
    
    👤 Klient: {klient_info['Nazwa']} (NIP: {klient_info['NIP']})
    📞 Telefon: {klient_info['Telefon']}
    
    💰 Brakuje do progu: {brakuje:.2f} zł
    (Jest to mniej niż {LIMIT_INTERWENCJI} zł)
...
def analizuj_pdf_i_szukaj_nipu(uploaded_file):
    ...
    moj_nip = "7722420459" # NIP GEKO - żeby go ignorować
    klient_nip = None
...
# --- UI APLIKACJI ---
st.set_page_config(page_title="GEKO Sales", page_icon="📈")
st.title("📈 GEKO Sales Booster")

df_klienci = load_clients()
if df_klienci is None:
    st.warning("⚠️ Wgraj plik baza_klientow.csv na GitHub!")
...
try:
    ...
    st.error("Skonfiguruj hasła w Secrets!")
    EMAIL_NADAWCY = ""
...
if uploaded_file:
    ...
    st.metric("Wartość zamówienia", f"{wartosc_netto} zł")

    brakujaca = PROG_KWOTOWY - wartosc_netto
    
    if 0 < brakujaca <= LIMIT_INTERWENCJI:
        rekomendacja = "Domów: Ostrzałka do łańcuchów (G81207)"
        st.error(f"🔥 Okazja! Brakuje tylko {brakujaca:.2f} zł")
        
        if klient_info['Telefon'] != "Brak":
            st.markdown(f"### 📞 [ZADZWOŃ TERAZ: {klient_info['Telefon']}](tel:{klient_info['Telefon']})")
        ...
        st.info("Za dużo brakuje (>300zł). Odpuszczamy.")
    else:
        st.success("Promocja już zdobyta!")
